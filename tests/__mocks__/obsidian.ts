...
export class Plugin {
	...
	addStatusBarItem(): HTMLElement {
		...
		const mockClassList = {
			...
			item: jest.fn(
				(index: number) => Array.from(classList)[index] || null,
			),
			...
			forEach: jest.fn(
				(
					...
					Array.from(classList).forEach((value, index) =>
						callback(value, index, mockClassList),
					);
				},
			),
		...
		element.querySelector = jest.fn((selector: string) => {
			if (selector === ".pomodoro-icon") {
				const iconEl = childElements.find(
					(el) =>
						el.classList &&
						typeof el.classList.contains === "function" &&
						el.classList.contains("pomodoro-icon"),
				);
			...
			if (selector === ".pomodoro-text") {
				const textEl = childElements.find(
					(el) =>
						el.classList &&
						typeof el.classList.contains === "function" &&
						el.classList.contains("pomodoro-text"),
				);
	...
	addCommand = jest.fn(
		(command: { id: string; name: string; callback: () => void }) => {
...
if (typeof document === "undefined") {
	global.document = {
		...
		createElement: (tagName: string) => Partial<HTMLElement>;
	};
}
