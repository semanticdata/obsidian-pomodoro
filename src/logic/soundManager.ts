...
export class SoundManager {
	...
	private async getSoundUrl(soundName: string): Promise<string> {
		...
		if (!soundName.startsWith('http://') && !soundName.startsWith('https://')) {
			try {
				// Read the file from the vault as binary data
				const arrayBuffer = await this.plugin.app.vault.adapter.readBinary(soundName);
				...
				console.error(`Failed to load vault audio file: ${soundName}`, error);
